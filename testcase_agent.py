...
async def generate_hidden_testcases(title: str, description: str, constraints: str, num_cases: int = 5) -> list[dict]:
    """
...
Generate exactly {num_cases} hidden test cases that evaluate maximum limits, minimum limits, empty/null states (if allowed), and tricky logic edge cases."""
    ...
    try:
        response = client.chat.completions.create(
            ...
            response_format={"type": "json_object"} # We will prompt for a wrapped object to ensure valid JSON from standard parsing if needed, actually dict wrapping is safer for response_format
        )
